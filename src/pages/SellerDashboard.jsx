...
const SellerDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch stats
      const statsResponse = await axios.get('http://localhost:5000/api/users/admin/stats', { headers });
      const productsResponse = await axios.get('http://localhost:5000/api/products/admin/all', { headers });
      const ordersResponse = await axios.get('http://localhost:5000/api/orders/admin/all', { headers });
      const usersResponse = await axios.get('http://localhost:5000/api/auth/admin/users', { headers });
  ...
  const updateProductStatus = async (productId, isAvailable) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:5000/api/products/admin/${productId}/status`, 
        { isAvailable }, 
        { headers: { Authorization: `Bearer ${token}` } }
      );
